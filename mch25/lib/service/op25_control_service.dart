...
class Op25ControlService extends ChangeNotifier {
  ...
  Future<void> attemptAutoStart() async {
    ...
    final flags = buildFlagsFromPrefs(prefs);

    if (flags != null) {
      debugPrint("Attempting to auto-start OP25 with saved config...");
      await startOp25WithFlags(flags);
    } else {
      debugPrint("OP25 auto-start skipped: No saved config found.");
    }
  }
